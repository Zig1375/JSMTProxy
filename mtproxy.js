...
getTelegramServers(start);

function start(telegram_servers) {
    console.log('Started');
    ...
    function create_idle_server(id, server) {
    ...
    setInterval(() => {
        ...
        for (let i = 0; i < telegram_servers.length; i++) {
            ...
            telegram_servers[i].idle = (n >= 5) ? n : 5;
            con_count[i] = 0;
        }
    }, REPORT_CON_SEC * 1000);
    ...
    setInterval(() => {
        ...
        for (var i = 0; i < server_count; i++) {
            if (server_idle_cons[i].length < telegram_servers[i].idle) {
...
function getTelegramServers(callback) {
    let servers = fs.readFileSync('./servers.txt').toString().split('\n');
    let list = [];

    servers.forEach((server) => {
        server = server.replace(/#.*/, '').trim();
        if (server) {
            let m = server.match(/^proxy_for ([0-9]+) ([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+):([0-9]+);$/);
            if ((m) && (m.length >= 4) && (m[1].indexOf('-') === -1)) {
                list.push({
                    ip: m[2],
                    port: parseInt(m[3]),
                    idle: MIN_IDLE_SERVERS
                });
            }
        }
    });

    // Пингуем и отбираем только самые быстрые сервера
    let result = [];
    ...
    let afterAll = () => {
        if (--count === 0) {
            ...
            // Отбираем 5 самых близких серверов
            list = list.slice(0, 5);
            let ms = list[0].ms * 3;
            for(let i = 4; i > 0; i--) {
                if (list[i].ms > ms) {
                    // Сервер медленнее самого первого в 3 раза, удаляем его нах
                    list.splice(i, 1);
                }
            }
            callback(list);
